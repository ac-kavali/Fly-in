        ...
        for line in file:
            if line == '\n' or line.startswith("#"):
                ...
                if i == 0 and not line.startswith("nb_drones"):
                    ...
                    absolut_num = line_content.replace("-", "") if line_content.startswith('-') else line_content
                    if not absolut_num.isdigit():
                        raise ConfigFileError("The number of drones support only number!")
                    if int(line_content) < 0:
                        raise ConfigFileError("The number of drones support only positive numbers")
                    if int(line_content) == 0:
                        raise ConfigFileError("The number of drones must be different to 0")
                    i += 1

                elif line.startswith("start_hub"):
                    if data_checklist["start_hub"] == 0:
                        data_checklist["start_hub"] += 1
                        start_hub = {"name": None, "x": None, "y": None}
